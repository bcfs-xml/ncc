...
// Thread-safe random number generator for OpenMP
#ifdef _OPENMP
static unsigned int rand_seed_array[256];
static int rand_seed_initialized = 0;

void init_thread_seeds(unsigned int base_seed) {
    for (int i = 0; i < 256; i++) {
        rand_seed_array[i] = base_seed + i * 12345;
    }
...
int thread_safe_rand() {
    ...
    if (tid >= 256) tid = tid % 256;

    // Xorshift algorithm
    unsigned int x = rand_seed_array[tid];
    x ^= x << 13;
    x ^= x >> 17;
    x ^= x << 5;
    rand_seed_array[tid] = x;

    return (int)(x & 0x7FFFFFFF);
}

#define RAND_FUNC thread_safe_rand
#else
...
Point find_best_position_fast(Piece* piece, Board* board) {
    ...
    for (int i = 0; i < board->piece_count; i++) {
        ...
        for (int j = i + 1; j < board->piece_count; j++) {
            ...
            double ot_min_y = other->rotated_piece.min_y + other->position.y;
...
Genome create_random_genome() {
    ...
    for (int piece_id = 0; piece_id < input_data.piece_count; piece_id++) {
        ...
        genome.rotation_choices[piece_id] = RAND_FUNC() % angle_count;
    }
...
void evaluate_genome(Genome* genome) {
    // Usar variavel local para thread-safety (OpenMP)
    Result local_result;
...
int tournament_selection(Genome* population, int pop_size) {
    int best_idx = RAND_FUNC() % pop_size;
    double best_fitness = population[best_idx].fitness;

    for (int i = 1; i < TOURNAMENT_SIZE; i++) {
        int candidate_idx = RAND_FUNC() % pop_size;
...
Genome order_crossover(Genome* parent1, Genome* parent2) {
    ...
    int cut1 = RAND_FUNC() % input_data.piece_count;
    int cut2 = RAND_FUNC() % input_data.piece_count;
    if (cut1 > cut2) {
        int temp = cut1;
        cut1 = cut2;
        cut2 = temp;
...
void mutate_genome(Genome* genome) {
    ...
    int num_swaps = 2 + RAND_FUNC() % 3;  // 2-4 swaps
    for (int m = 0; m < num_swaps; m++) {
        if ((double)RAND_FUNC() / RAND_MAX < MUTATION_RATE) {
            int pos1 = RAND_FUNC() % input_data.piece_count;
            int pos2 = RAND_FUNC() % input_data.piece_count;
    ...
    int num_rotations = 3 + RAND_FUNC() % 4;  // 3-6 rotações
    for (int m = 0; m < num_rotations; m++) {
        if ((double)RAND_FUNC() / RAND_MAX < MUTATION_RATE) {
            int piece_id = RAND_FUNC() % input_data.piece_count;
            int angle_count = input_data.pieces[piece_id].angle_count;
            if (angle_count > 1) {
                genome->rotation_choices[piece_id] = RAND_FUNC() % angle_count;
            }
...
// Versao especial que popula o result global (para save_best_result)
void evaluate_genome_and_save(Genome* genome) {
    if (result.boards) {
...
int main(int argc, char* argv[]) {
    ...
    // Configurar OpenMP para usar todos os cores disponíveis
    #ifdef _OPENMP
    int max_threads = omp_get_max_threads();
    omp_set_num_threads(max_threads);
    printf("[OpenMP] Paralelizacao habilitada com %d threads\n", max_threads);
    printf("[OpenMP] Processadores disponiveis: %d\n\n", omp_get_num_procs());
    #else
    printf("[AVISO] OpenMP NAO esta habilitado! Compilacao sera sequencial.\n\n");
    #endif
    ...
    for (int i = 0; i < greedy_count; i++) {
        population[idx++] = create_greedy_genome();
    }

    // 90% aleatório
    while (idx < POPULATION_SIZE) {
        population[idx++] = create_random_genome();
    }
    ...
    printf("Avaliacao inicial concluida!\n");
    ...
    evaluate_genome_and_save(&population[best_idx]);
    save_best_result();
    ...
    for (int gen = 0; gen < GENERATIONS; gen++) {
        ...
        if (population[0].fitness > current_best_fitness) {
            evaluate_genome_and_save(&population[0]);
            save_best_result();
        }
